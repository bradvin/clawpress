    ...
    <form className="clawpress-tool-dialog-form" onSubmit={handleSubmit}>
      {fields.map((field) => {
        ...
        if (field.type === 'hidden') {
          return <input key={field.name} {...common} type="hidden" value={value ?? ''} />;
        }

        return (
          <label key={field.name} className="clawpress-tool-dialog-field" htmlFor={common.id}>
            <span className="clawpress-tool-dialog-label">{field.label}</span>
            {field.type === 'textarea' ? (
              <textarea
                {...common}
            ...
            {field.help ? (
              <span className="clawpress-tool-dialog-help">{field.help}</span>
            ) : null}
          </label>
        );
      })}
