...
const Panel = () => {
	const [ open, setOpen ] = useState(
		JSON.parse( localStorage.getItem( 'clawpress_open' ) || 'false' )
	);
	...
	const buildErrorCard = ( message, type = '' ) => {
		...
		const subtitle =
			typeof type === 'string' && subtitleByType[ type ]
				? subtitleByType[ type ]
				: __( 'Error', 'clawpress' );
	...
	const formatToolCallMessage = ( rawCall, index = null, total = null ) => {
		...
		const callStatus =
			typeof rawCall.status === 'string'
				? rawCall.status.trim().toLowerCase()
				: 'success';
		const callMessage =
	...
	const normalizePanelState = ( state ) => {
		...
		const nextWidth = Number.isFinite( Number( state.width ) )
			? Number( state.width )
			: null;
		const lastHistoryId =
			typeof state.last_history_id === 'string'
				? state.last_history_id
				: '';
		const welcomeCardSeen =
			typeof state.welcome_card_seen === 'boolean'
				? state.welcome_card_seen
				: false;
	...
	useEffect(
		() => localStorage.setItem( 'clawpress_width', String( width ) ),
		[ width ]
	);
	useEffect( () => {
	...
	const getToolConcurrency = ( toolName ) =>
		getToolPolicy( toolName ).concurrency || 'parallel';
	...
	const finishStream = () => {
		...
		const plans = Array.isArray( toolPlansRef.current )
			? toolPlansRef.current
			: [];
		if ( plans.length > 0 ) {
			setToolDialogs( ( prev ) => {
				const next = [ ...prev ];
		...
		switch ( eventType ) {
			...
			case 'tool_call':
				...
				if ( ! toolPlanningShown ) {
					setEphemeralStatus(
						__( 'Preparing tool plan…', 'clawpress' )
					);
			...
			case 'error':
				appendMessage(
					...
					parsed?.card ||
						buildErrorCard( parsed?.error, parsed?.type )
				);
				break;
			case 'done':
				finishStream();
	...
	useEffect( () => {
		...
		const initializePanelState = async () => {
			...
			try {
				...
				if (
					Number.isFinite( resolvedPanelState.width ) &&
					resolvedPanelState.width > 0
				) {
			...
			try {
				...
				if ( shouldShowWelcomeCard ) {
					...
					client
						.setPanelState?.( { welcome_card_seen: true } )
						.catch( () => {} );
				} else {
					setMessages( historyMessages );
					timelineRef.current = historyMessages.reduce(
						( max, item ) =>
							Math.max( max, Number( item.createdAt ) || 0 ),
						0
					);
				}
			} catch {
	...
	useEffect( () => {
		...
		const payload = {
			...
			width,
			last_history_id:
				typeof lastMessage?.id === 'string' ? lastMessage.id : '',
		};
	...
	const updateToolDialog = ( id, updater ) => {
		setToolDialogs( ( prev ) =>
			prev.map( ( dialog ) =>
				dialog.id === id ? { ...dialog, ...updater } : dialog
			)
		);
	};
	...
	const executeToolDialog = async ( dialog ) => {
		...
		try {
			...
			const diff = Array.isArray( result?.changed )
				? result.changed
				: null;
			...
			updateToolDialog( dialog.id, {
				status: 'error',
				error:
					err?.message || __( 'Tool execution failed.', 'clawpress' ),
			} );
		}
	};
	...
	const requestRunToolDialog = ( dialogId, overrideArgs ) => {
		...
		const nextDialog = overrideArgs
			? { ...dialog, args: overrideArgs }
			: dialog;
	...
	const handleCardAction = ( action ) => {
		...
		if ( action.type === 'open_url' ) {
			...
			if ( ! url ) {
				appendMessage(
					'system',
					__( 'Invalid card action.', 'clawpress' )
				);
				return;
			}
	...
	return (
		<Fragment>
			{ showFloatingToggle ? (
				<PanelToggle onToggle={ () => setOpen( ( o ) => ! o ) } />
			) : null }
			<div
