...
export const ToolDialogShell = ({
  ...
  const showActions =
    showActionsOverride !== undefined
      ? showActionsOverride
      : status !== 'running' && !isCancelled && (status !== 'done' || canRerun);
  const isBlocked = status === 'blocked';
  const statusMessage = getStatusMessage(status, policy);

  return (
    <details className="clawpress-tool-dialog" open={isCancelled ? false : isOpen}>
      <summary className="clawpress-tool-dialog-summary">
        <span className="clawpress-tool-dialog-heading">
          <span
            ...
            aria-hidden="true"
          />
          <span className="clawpress-tool-dialog-title">{title}</span>
        </span>
        {showActions ? (
          <span className="clawpress-tool-dialog-actions">
            {showErrorIcon ? (
              <span className="clawpress-tool-dialog-error-icon" aria-hidden="true">
            ...
            <button
              ...
              type="button"
              onClick={onRun}
              disabled={status === 'running' || isBlocked || isCancelled}
            >
              {runLabel}
            </button>
