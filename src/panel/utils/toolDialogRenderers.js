...
const ToolDefaultRenderer = ({ toolName, args, toolDialog, runTool, onCancel, policy, isOpen }) => {
  ...
  return (
    ...
    >
      {status === 'done' ? (
        <syntax-highlight language="json">{JSON.stringify(result, null, 2)}</syntax-highlight>
      ) : (
        <syntax-highlight language="json">{JSON.stringify(args, null, 2)}</syntax-highlight>
      )}
      {toolDialog.diff ? (
...
export const getToolPolicy = (toolName) =>
