...
Implement chat-led setup state machine which guides the user through the initial setup process. It shold provide a nice UX and explain everything that is happening.

## Source Requirements

- Plugin spec: 5.2, 6.3, 9, 15.2, 15.8, 15.13
- Agent spec: Level 0 setup constraints and section 3 request lifecycle step 4

## In Scope

- Setup state machine storage and progression.
- Provider and model selection. If providers not setup then include link to `wp-admin/options-general.php?page=wp-ai-client`. Setup is blocked.
...
- Rich conversational setup copy generation by LLM.
- Multi-site setup variations.

## Implementation Tasks

1. Setup module
- Add `includes/setup.php` with state transitions and resumable progress.
- Persist setup state to settings.

2. Setup card which is a wizard that guides the user through the setup process.
...
- Track template version via `clawpress_setup_templates_version`.

5. UI integration
- Add `SetupFlow` component to guide first-run steps and resume.
...
- If providers not setup then include link to `wp-admin/options-general.php?page=wp-ai-client`. Setup is blocked. Check if the user is on that page, and if so show a refresh button in the wizard to get the providers. The provider helper should be used under the hood.
...
- Fresh install enters setup and can resume after reload.
- Setup cannot complete until settings are valid.
...
- Integration: setup REST progression and idempotent re-run.
